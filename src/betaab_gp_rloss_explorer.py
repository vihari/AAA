...
class BetaGPExplorer(torch.nn.Module):
    def __init__(self, dataset: dataset.Dataset, fitter: data_fitter.Fitter, cache_dir: str, 
        ...
        self.gp_likelihood = beta_gp_likelihood.BetaABLikelihood(self.width, no_scale_loss=self.no_scale_loss, dw_alpha=self.dw_alpha, rloss_approx_type=self.rl_approx_type).to(self.dev)
    ...
    def mean_variance(self, debug=False):
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            def mean_fn(sample):
                alpha = misc.a_transform(sample[:, :, 0])
                beta = misc.b_transform(sample[:, :, 1])
...
def estimation_ablation(args, kwargs):
    ...
    save_name = "%s/betaab_gp_rloss_explorer" % args[1].cache_dir
    save_name += "_exp=" + ustr
    save_name += "_seed=%d_estimation_ablation.pkl" % seed
